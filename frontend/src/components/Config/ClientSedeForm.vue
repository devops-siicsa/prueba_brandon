        ...
        <div class="d-flex align-center">
            ...
            <div>
                <h2 class="text-h6 font-weight-bold text-grey-darken-3" style="line-height: 1.2;">
                    {{ sede ? 'Editar Sede de Cliente' : 'Registrar Sede de Cliente' }}
                </h2>
                <p class="text-caption text-grey mt-1">
                    Complete la información para {{ sede ? 'actualizar la' : 'dar de alta una nueva' }} sede operativa.
                </p>
            </div>
        </div>
      ...
      <v-card-text class="px-6 py-2 scroll-container" :style="isMobileApp ? 'height: calc(100vh - 140px); overflow-y: auto;' : 'max-height: 60vh; overflow-y: auto;'">
        <v-form ref="form" @submit.prevent="save">
            <v-row dense>
                <!-- Cliente (Empresa) -->
                <v-col cols="12">
                    <label class="text-caption font-weight-bold text-grey-darken-1 mb-1 d-block">CLIENTE ASOCIADO</label>
                    <v-autocomplete
                ...
                <v-col cols="12">
                    ...
                    <v-text-field 
                        ...
                        placeholder="Ej. Sede Principal, Planta Norte..." 
                        variant="outlined" 
                        density="compact"
...
// Filtrar solo empresas que son clientes
const clientCompanies = computed(() => {
    return props.companies.filter(c => c.EsCliente === true)
})

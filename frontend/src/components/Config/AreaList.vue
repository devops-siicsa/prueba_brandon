      ...
      <div class="px-8 pt-8 pb-4 d-flex justify-space-between align-start">
        <div class="d-flex align-center">
            <div class="header-icon-box mr-4">
                <v-icon color="primary" size="32">mdi-chart-tree</v-icon>
            </div>
            <div>
                <h2 class="text-h5 font-weight-bold text-grey-darken-3 mb-1">Gestión de Áreas</h2>
                <p class="text-body-2 text-grey">Administra las dependencias y departamentos de la empresa.</p>
            </div>
        </div>
        ...
        <v-card class="rounded-xl">
            <v-card-title class="px-6 pt-6 pb-2 d-flex align-center justify-space-between">
                <span class="text-h6 font-weight-bold text-grey-darken-3">
                    {{ editedItem.Id ? 'Editar Área' : 'Nueva Área' }}
                </span>
...
async function loadItems() {
    loading.value = true
    try {
        const res = await axios.get('http://localhost:5000/api/config/catalogs/areas', { withCredentials: true })
...
async function save() {
    ...
    saving.value = true
    try {
        // Nota: El backend usa endpoints genéricos de catálogo, pero podríamos necesitar uno específico si la lógica crece.
        // Por ahora usamos el genérico de catálogos.
        const url = 'http://localhost:5000/api/config/catalogs/areas'
        
        // El backend actual de catálogos genéricos no soporta PUT/Update por ID en la ruta genérica de forma estándar
        // según lo que vi en CatalogManager. Pero UserForm y CompanyForm sí.
        // Asumiré que para catálogos simples solo hay POST (Crear) por ahora en el backend genérico, 
        // OJO: Si necesito editar, debo verificar si el backend soporta PUT en /catalogs/areas/<id> o similar.
        // Revisando config_routes.py:
        // @config_bp.route('/catalogs/<catalog_name>', methods=['POST']) -> Crear
        // NO HAY PUT para catálogos genéricos.
        // Esto es una limitación. Para editar, necesitaría implementar el endpoint en el backend.
        // Por ahora, implementaré solo CREAR. Si es edición, mostraré un aviso o implementaré el endpoint.
...
function confirmDelete(item) {
    if (confirm(`¿Estás seguro de eliminar el área "${item.Nombre}"?`)) {
        // TODO: Implementar delete en backend
        alert("La eliminación aún no está implementada en el backend.")
    }
}

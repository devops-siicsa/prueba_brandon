<template>
  <v-dialog 
    v-model="dialog" 
    :max-width="isMobileApp ? '100%' : '1100'" 
    :fullscreen="isMobileApp"
    :transition="isMobileApp ? 'dialog-bottom-transition' : 'dialog-transition'"
    scrollable
  >
    <v-card class="rounded-xl elevation-24" :class="{'rounded-0': isMobileApp}">
      <!-- Header Personalizado -->
      <div class="px-8 pt-8 pb-4 d-flex justify-space-between align-start">
        <div>
            <h2 class="text-h5 font-weight-bold text-grey-darken-3 mb-1">Sedes de Clientes</h2>
            <p class="text-body-2 text-grey">Gestiona las ubicaciones y oficinas de tus clientes.</p>
        </div>
      ...
      <v-card-text class="px-8 py-6 bg-grey-lighten-5" :style="isMobileApp ? 'height: calc(100vh - 140px);' : ''">
        ...
        <div v-else-if="filteredSedes.length === 0" class="d-flex flex-column align-center justify-center py-12 text-center">
            <div class="empty-state-icon mb-6">
                <v-icon size="48" color="primary">mdi-office-building-marker-outline</v-icon>
            ...
            <p class="text-body-2 text-grey mb-8" style="max-width: 400px;">
                No hay sedes de clientes registradas que coincidan con tu búsqueda.
            </p>
                ...
                >
                    ...
                    <v-card-text class="pa-5">
                        ...
                        <div class="text-caption text-grey-darken-1 mb-2 font-weight-medium d-flex align-center">
                            <v-icon size="small" start class="mr-1">mdi-account-tie</v-icon>
...
const filteredSedes = computed(() => {
    // Primero filtrar solo sedes de clientes
    let result = sedes.value.filter(s => {
        const comp = companies.value.find(c => c.Id === s.EmpresaId)
        return comp && comp.EsCliente === true
    })

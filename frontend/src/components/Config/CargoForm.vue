    ...
    <v-card class="rounded-xl" :class="{'rounded-0': isMobileApp}">
        <v-card-title class="px-6 pt-6 pb-2 d-flex align-center justify-space-between">
            <span class="text-h6 font-weight-bold text-grey-darken-3">
                {{ cargo.Id ? 'Editar Cargo' : 'Nuevo Cargo' }}
            </span>
        ...
        <v-card-text class="px-6 py-4">
            <v-form ref="form" @submit.prevent="save">
                <label class="text-caption font-weight-bold text-grey-darken-1 mb-1 d-block">NOMBRE DEL CARGO</label>
                <v-text-field
...
async function save() {
    ...
    loading.value = true
    try {
        const url = '/api/config/catalogs/cargos'
        
        if (props.cargo.Id) {
            await axios.put(`${url}/${props.cargo.Id}`, formData.value, { withCredentials: true })
        } else {
